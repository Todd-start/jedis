...
public class TechwolfBinaryJedisCluster implements BasicCommands, BinaryJedisClusterCommands,
  ...
  protected TechwolfJedisClusterConnectionHandler connectionHandler;

  public TechwolfBinaryJedisCluster(Set<HostAndPort> jedisClusterNode,String password,String clinetName,GenericObjectPoolConfig poolConfig){
    this(jedisClusterNode,DEFAULT_TIMEOUT,DEFAULT_TIMEOUT,DEFAULT_MAX_REDIRECTIONS,password,clinetName,poolConfig);
  }
  ...
  public byte[] get(final byte[] key) {
    return new TechwolfJedisClusterCommand<byte[]>(connectionHandler, maxAttempts) {
      @Override
  ...
  public List<byte[]> hmget(final byte[] key, final byte[]... fields) {
    return new TechwolfJedisClusterCommand<List<byte[]>>(connectionHandler, maxAttempts) {
      @Override
  ...
  public Set<byte[]> hkeys(final byte[] key) {
    return new TechwolfJedisClusterCommand<Set<byte[]>>(connectionHandler, maxAttempts) {
      @Override
  ...
  public Collection<byte[]> hvals(final byte[] key) {
    return new TechwolfJedisClusterCommand<Collection<byte[]>>(connectionHandler, maxAttempts) {
      @Override
  ...
  public Map<byte[], byte[]> hgetAll(final byte[] key) {
    return new TechwolfJedisClusterCommand<Map<byte[], byte[]>>(connectionHandler, maxAttempts) {
      @Override
  ...
  public ScanResult<byte[]> scan(final byte[] cursor, final ScanParams params) {
    ...
    if (params == null || (matchPattern = params.match()) == null || matchPattern.isEmpty()) {
      throw new IllegalArgumentException(TechwolfBinaryJedisCluster.class.getSimpleName() + " only supports SCAN commands with non-empty MATCH patterns");
    }

    if (JedisClusterHashTagUtil.isClusterCompliantMatchPattern(matchPattern)) {

      return new TechwolfJedisClusterCommand< ScanResult<byte[]>>(connectionHandler,
              maxAttempts) {
        @Override
      ...
      throw new IllegalArgumentException(TechwolfBinaryJedisCluster.class.getSimpleName() + " only supports SCAN commands with MATCH patterns containing hash-tags ( curly-brackets enclosed strings )");
    }
  }
  
  @Override
  public ScanResult<Map.Entry<byte[], byte[]>> hscan(final byte[] key, final byte[] cursor) {
    return new TechwolfJedisClusterCommand<ScanResult<Map.Entry<byte[], byte[]>>>(connectionHandler,
        maxAttempts) {
      @Override
  ...
  public ScanResult<byte[]> sscan(final byte[] key, final byte[] cursor) {
    return new TechwolfJedisClusterCommand<ScanResult<byte[]>>(connectionHandler, maxAttempts) {
      @Override
      public ScanResult<byte[]> execute(Jedis connection) {
